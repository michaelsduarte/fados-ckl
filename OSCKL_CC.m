function [expansionCoefficient,biasCoefficient,dictionaryIndex,learningCurve,identOutlier] = ...
    OSCKL_CC(trainInput,trainTarget,testInput,testTarget,typeKernel,paramKernel,regularizationFactor,regularizationFactor2,forgettingfactor,toleranceCoherence,flagLearningCurve,ks,epson_updt)
...
for i = 2:trainSize
    ...
    coherence = max(ker_eval(trainInput(:,i),trainInput(:,dictionaryIndex),typeKernel,paramKernel));
    ...
    dictSize = dictSize + 1;
    disp(['CC-Dictsize...',num2str(dictSize)])
