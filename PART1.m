...
s2 = var(ySNL)*50; %Desvio da gaussiana 2
...
for no = 1:nOutlier
    ...
    for nt = 1:nTrial
        ...
        if no == 1
            th_ald_vector = linspace(0.006,0.006,length_ald);
            %th_ald_vector = 0.006; %0.006; Poly = 3; 0.03; Gauss=2;
            ...
            th_ald_vector = linspace(0.006,0.006,length_ald);
            %th_ald_vector = 0.005;%0.0005; Poly = 3;
        ...
        %th_ald_vector = linspace(0.0055,0.0055,length_ald);
        if no == 1
            th_distance_nc_vector = linspace(.15,.15,length_nc); %0.15
            ...
            %th_distance_nc_vector = linspace(0.008,0.008,length_nc);
            th_error_nc_vector  = linspace(0.001,0.001,length_nc); %0.0025
...
%             th_error_nc_vector = .003;%.004; Poly = 3
        ...
        %th_ald_vector = linspace(0.0055,0.0055,length_ald);
        if no == 1
            th_oscklcc = linspace(1.01,1.01,length_cc);
            %th_oscklcc = 1.014;%1.01 = 170 SVs and 1.015 = 420 SVs;Poly = 3; .95; Gauss=2;
            ks = .2; % Bandwidth correntropy
            epson_updt = .00001; % Criterion update matrix
            regularizationFactor = 10; % Regularization parameter correntropy
        else
        ...
        %th_ald_vector = linspace(0.0055,0.0055,length_ald);
        if no == 1
            th_oscklsc = linspace(-3.1,-3.1,length_sc);%-.29 outro criterio no codigo
            %th_oscklsc = -.3; % -.3 = 390 SVs; Poly = 3;-1.5; Gauss=2;
            ks = .2; % Bandwidth correntropy
            epson_updt = .00001; % Criterion update matrix
            regularizationFactor = 10; % Regularization parameter correntropy
            
        else
            th_oscklsc = linspace(-2.7,-2.7,length_sc);%-.29
            %th_oscklsc = -.3;
            ks = .4; % Bandwidth correntropy
            epson_updt = .00001; % Criterion update matrix
            regularizationFactor = 10; % Regularization parameter correntropy
